...
\begin{tabular} {|| r | l | p{5cm} ||}
  ...
  $(p : \Z^2) \to \Z^2$ &
  Converts a point to its grid index (the identity mapping under the current set of restrictions).
  \\
  \hline
  \texttt{min\_dist\_pair\_aux} &
  ...
  $(n : \N) \to [\Z^2]$ &
  Given a radius $n$, returns the set of grid cell indices that intersect a hypercube of radius $n$.
  \\
  \hline
  \texttt{get\_idxs} &
  $(p : \Z^2) \to (c : \N^+) \to [\Z^2]$ &
  Returns a list of indices in a hypercube of radius $\sqrt{c}^\N + 1$ around $p$, making use of \texttt{get\_hypercube}.
  \\
  \hline
  \texttt{lift\_option\_list} &
  $\forall (\alpha : \mathsf{Type}).\, [\alpha]^? \to [\alpha]$ &
  Converts all $\mathsf{none}$ lists into empty lists.  \\
  \hline
  \texttt{get\_neighbs} &
  $(p : \Z^2) \to (g : \mathsf{Grid}) \to [\Z^2]$ &
  Uses \texttt{get\_idxs} to collect indices around $p$, then fetches cells at those indices from $g$, excludes $p$ from the result, then returns.
  \\
  \hline
\end{tabular}
\end{center}
...
% Lean is perhaps best suited to the formalization of mathematics.



% When mathematics and programming are put on equal foundations (i.e., type theory), as they are in Lean, it becomes clear that the constructs in algorithms are often as complex (or more) than constructions in mathematics, and they are far more bespoke.

% and formal verification should only be employed for the most critical components of digital infrastructure.
