...
| (p :: ps) :=
    ...
    let bucket_idx := get_bucket_idx p t in
    let N := get_neighbs p g bucket_idx in
...
theorem find_closest_pair_correct :
  ∀ (t : ℕ) (t_nonzero : t ≠ 0) (ps : list point),
    (∃ (p : point) (q : point),
      (closest_pair p q ps) ∧ (1 < ∥ p - q ∥) ∧ (∥ p - q ∥ ≤ t)) →
      (∃ (p : point) (q : point),
        (find_closest_pair t t_nonzero ps) = some (p, q) ∧
        closest_pair p q ps) :=
begin
  intros t t_nonzero ps exists_pair,
  apply exists.intro,
  apply exists.intro,
